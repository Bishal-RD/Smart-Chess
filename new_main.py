...
from openingDB import get_opening_moves, convert_move_to_tuple
...
def main():
    ...
    while True:
        ...
        if turn == human_color:
            ...
            available_moves = get_opening_moves(fen)
            print(available_moves)
            try:
                # Attempt to get the AI move from the first available move
                ai_move = convert_move_to_tuple(available_moves[0])
            except IndexError:
                # Handle the case where available_moves is empty
                print("Error: No available moves for the AI to select from.")
                ai_move = None  # Assign a default value or handle appropriately
            except Exception as e:
            ...
            if not ai_move:
                ai_move = iterative_deepening(board, ai_color, last_move, max_depth=2, time_limit=120)
                print(ai_move)
